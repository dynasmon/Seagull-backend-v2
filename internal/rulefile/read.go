...
func Parse(source string, data []byte) ([]*detection.Program, error) {
...
func parse(source string, data []byte, seen map[detection.ID]string) ([]*detection.Program, error) {
	...
	programs := make([]*detection.Program, 0, len(list.Content))
	for _, node := range list.Content {
		program, err := compiled(read, node, seen)
		if err != nil {
			return nil, err
		}
...
func compiled(read *reader, node *yaml.Node, seen map[detection.ID]string) (*detection.Program, error) {
	rule, err := read.rule(node)
	if err != nil {
		return nil, err
	}
	...
	if first, twice := seen[rule.ID]; twice {
		return nil, read.fault(read.locate("id", node), "id",
	...
	if err != nil {
		return nil, read.refused(node, err)
	}
